...
class PW3D_Text2MotionDatasetV2(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.opt.load_mode == 'prefix':
            ...
            upper_bound = min(self.max_motion_length, orig_length)
            if self.min_motion_len >= upper_bound:
                m_length = upper_bound
            else:
                m_length = random.randint(self.min_motion_len, upper_bound)
        if orig_length - m_length <= 0:
            offset = 0
        else:
